...
namespace CommandBuilder.Command
{
    ...
    /// 格式
    /// </summary>
   public class G117 : ICmd
    {
        private string name = "G117";
        public List<G117Param> paramList = new List<G117Param>();

        //解析参数,因为入栈,所以倒序输出
        public void ParserParam(string cmdstring)
        {
            paramList.Clear();
            Stack<G117Param> st = new Stack<G117Param>();//参数栈
            ...
            foreach (var item in cmdstring)//逐个字符解析参数
            {
                if (item == 'A')
                {
                    ...
                    p01 = new G117Param() { AValue = 0, EValue = 0, FValue = 0 };//遇到A,就新建一个参数
                    st.Push(p01);//入栈
                    statu = "A";
                }
                else if (item == 'E')
            ...
            G117Param endparam = st.Pop();//获取最后一个参数
